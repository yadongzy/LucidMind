...
class WebSocketChannelAdapter(ChannelPort):
    ...
    async def handle_connection(self, websocket: WebSocket, brain,
        ...
        current_raw_sid = "default"  # 本连接当前会话（chat 消息缺省 session_id 时兜底）
        ...
        try:
            while True:
                ...
                if msg_type == "server_pong":
                    ...
                    current_raw_sid = data.get("session_id", "default")
                    iso_sid = self._isolate_sid(user_id, current_raw_sid)
                    ...
                    if user_input:
                        raw_sid = data.get("session_id") or current_raw_sid
