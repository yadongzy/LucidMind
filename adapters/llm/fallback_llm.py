...
class FallbackLLMAdapter(LLMPort):
    ...
    def set_primary(self, adapter: LLMPort) -> None:
        ...
        # 新加入的 adapter 必须登记健康表，否则 chat() 里的
        # failures 计数会写进临时 dict 而丢失，冷却/恢复机制失效
        for a in self._all:
            name = getattr(a, "model", "?")
    ...
    async def chat(
        ...
        **kwargs,
        ...
        for i, adapter in enumerate(self._all):
            ...
            # setdefault：确保 failures 计数写回健康表，而不是写进临时 dict 丢失
            h = self._health.setdefault(model_name, {"failures": 0, "last_fail": 0, "cooldown": 0})
