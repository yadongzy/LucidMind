...
# 黑名单子串拦不住的变体（rm -fr /、rm -r -f /、sudo 无参数等）用规则补充。
# 注意：黑名单方式本质可绕过（python -c、find -delete 等），只作纵深防御。
_RECURSIVE_RM_RE = re.compile(r"(?:^|[;&|])\s*rm\s+(.+)", re.IGNORECASE)
_PRIVILEGE_RE = re.compile(r"\b(?:sudo|doas)\b", re.IGNORECASE)


def _matches_dangerous_rules(command: str) -> str | None:
    """返回命中的危险规则名，未命中返回 None。"""
    # rm 带递归+强制标志（标志可拆分/换序）且目标指向根/家目录/通配/当前目录
    for m in _RECURSIVE_RM_RE.finditer(command):
        tokens = m.group(1).split()
        flags = "".join(t.lstrip("-") for t in tokens if t.startswith("-")).lower()
...
class ShellAdapter(ToolPort):
    ...
    async def _run_command(self, command: str) -> dict[str, Any]:
        ...
        # S19-1: 安全检查 — 危险命令 + 敏感路径 + 规则引擎
        cmd_lower = command.lower()
        for blocked in _BLOCKED_PATTERNS:
            if blocked in cmd_lower:
                logger.warning(f"命令被拦截(危险): {command}")
                return {"success": False, "result": None, "error": f"命令被安全策略拦截: 包含危险操作 '{blocked}'"}
        rule_hit = _matches_dangerous_rules(command)
        if rule_hit:
            logger.warning(f"命令被拦截(规则): {command}")
            return {"success": False, "result": None, "error": f"命令被安全策略拦截: {rule_hit}"}
