...
class ToolSafetyGuard:
    ...
    async def _request_approval(self, session_id: str, tool_name: str,
        ...
        # 没有 WebSocket 连接（无头模式如 Daemon）：
        # SENSITIVE 自动放行；DANGEROUS 一律拒绝，防止无监督执行破坏性操作
        if not self._ws_channel or not self._ws_channel._connections:
            if level == "dangerous":
                logger.warning(f"无 WebSocket 连接，拒绝执行危险工具: {tool_name}")
                return {"approved": False, "reason": "无用户在线，危险操作需要用户确认"}
            logger.warning(f"无 WebSocket 连接，自动放行敏感工具: {tool_name}")
            return {"approved": True}
        ...
        try:
            ...
            # 超时 = 用户未确认 → 拒绝执行（安全默认），绝不自动放行或加白名单
            logger.warning(f"工具审批超时，拒绝执行: {tool_name}")
            return {"approved": False, "reason": "审批超时（60秒无响应）"}
        finally:
    ...
    def handle_approval_response(self, request_id: str, approved: bool, reason: str = "") -> bool:
        ...
        SENSITIVE 工具批准后自动加入白名单（后续免审批）；
        DANGEROUS 工具仅本次放行，每次执行仍需确认。
        """
        future = self._pending.get(request_id)
        if not future or future.done():
            return False
        if approved:
            # 从 request_id 提取工具名：approve_{timestamp}_{tool_name}
            m = re.match(r"approve_\d+_(.*)", request_id)
            if m:
                tool_name = m.group(1)
                if self.classify(tool_name) == "sensitive":
                    self._custom_safe.add(tool_name)
                    self._custom_dangerous.discard(tool_name)
                    self._save_config()
                    logger.info(f"敏感工具 {tool_name} 已加入白名单（后续免审批）")
                else:
                    logger.info(f"危险工具 {tool_name} 本次已批准（不加入白名单）")
