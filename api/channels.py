...
async def restart_channel(name: str):
    ...
    try:
        await ch.stop()
        # 复用适配器现有的消息回调；start(None) 会把回调覆盖为 None，
        # 导致重启后收到消息无人处理
        await ch.start(getattr(ch, "_on_message", None))
...
async def ngrok_start():
    ...
    # 注意：stdout 不能用 PIPE——ngrok 日志会写满 64KB 管道缓冲区后阻塞假死；
    # 状态一律通过 127.0.0.1:4040 本地 API 查询
    try:
