...
_scheduler_task: asyncio.Task | None = None  # 调度循环任务引用（防重复启动 + 防 GC）
...
def add_job(description: str, command: str, interval_seconds: int = 0,
    ...
    # WebSocket 推送通知前端（仅在事件循环内；同步上下文调用时跳过）
    if _ws_channel:
        try:
            loop = asyncio.get_running_loop()
            loop.create_task(_ws_channel.broadcast(json.dumps({
                "type": "cron_updated", "event": "created", "job": job,
            })))
        except RuntimeError:
            pass  # 无运行中的事件循环（如从线程/同步代码调用）
        except Exception:
            pass
    return job
...
async def update_cron_job(job_id: str, patch: dict):
    ...
    for job in _jobs:
        if job.get("id") == job_id:
            ...
            if job.get("type", "every") == "every":
                # every 类型：schedule/interval 变更时必须重算 interval_seconds，
                # 否则 _parse_interval 会因旧的 interval_seconds>0 而忽略新 schedule
                if "interval_seconds" in patch and int(patch["interval_seconds"] or 0) > 0:
                    job["interval_seconds"] = int(patch["interval_seconds"])
                    job["schedule"] = f"every {job['interval_seconds']}s"
                elif "schedule" in patch and patch["schedule"]:
                    resolved = _parse_interval(patch["schedule"], 0)
                    if resolved >= 60:
                        job["schedule"] = patch["schedule"]
                        job["interval_seconds"] = resolved
                    else:
                        raise HTTPException(400, f"无效的间隔: {patch['schedule']}（最小 60 秒）")
            elif "schedule" in patch and patch["schedule"]:
                job["schedule"] = patch["schedule"]
            _save_jobs()
...
async def start_cron_scheduler():
    """启动 cron 调度循环（支持 every + at 两种类型）。幂等：重复调用不会启动第二个循环。"""
    global _running, _scheduler_task
    if _scheduler_task and not _scheduler_task.done():
        logger.debug("Cron 调度器已在运行，跳过重复启动")
        return
...
def stop_cron_scheduler():
    """停止 cron 调度循环（大脑休眠时调用，防止再次唤醒后双循环并存）。"""
