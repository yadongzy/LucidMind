...
def _get_store():
    ...
    之前每个请求都新建连接并重建 FTS 表、用完即关，开销大且
    embed_fn 为空导致 API 搜索永远不走向量。改为进程级单例 +
    与 learning 适配器一致的向量配置。
    """
    ...
    if _store_singleton is None:
        ...
        _store_singleton = MemoryStore(
            db_path, cfg.embedding_dim,
            embed_fn=vec.embed if vec.is_available() else None,
        )
