...
class Brain(BrainResilienceMixin, BrainLearningMixin, BrainToolGuardMixin, BrainIntentMixin):
    ...
    # 并发隔离（ContextVar）：每个 asyncio 任务持有独立的 Context。
    # Brain 是跨连接单例，CHAT lane 允许 3 并发 + Daemon 任务同时运行，
    # 之前用实例属性存 _current_sid / 全局替换 llm/tools 会导致多任务互相串话。
    _sid_var = contextvars.ContextVar("brain_current_sid", default="default")
    _llm_override_var = contextvars.ContextVar("brain_llm_override", default=None)
    _tools_override_var = contextvars.ContextVar("brain_tools_override", default=None)
    ...
    # ── 任务级覆盖与当前会话（ContextVar 隔离）──

    @property
    def llm(self) -> LLMPort:
        """当前 LLM：任务级覆盖（如本地模型分流）优先，回退全局主适配器。"""
        return self._llm_override_var.get() or self._llm

    @llm.setter
    def llm(self, value: LLMPort) -> None:
        self._llm = value

    @property
    def tools(self) -> ToolPort | None:
        """当前工具集：任务级覆盖（如工具分组代理）优先，回退全局工具集。"""
        return self._tools_override_var.get() or self._tools

    @tools.setter
    def tools(self, value: ToolPort | None) -> None:
        self._tools = value

    @property
    def _current_sid(self) -> str:
        return self._sid_var.get()

    @_current_sid.setter
    def _current_sid(self, value: str) -> None:
        self._sid_var.set(value)

    def set_llm_override(self, adapter: LLMPort):
        """为当前任务临时切换 LLM（返回 token，用 reset_llm_override 恢复）。"""
        return self._llm_override_var.set(adapter)

    def reset_llm_override(self, token) -> None:
        self._llm_override_var.reset(token)

    def set_tools_override(self, tools: ToolPort):
        """为当前任务临时替换工具集（返回 token，用 reset_tools_override 恢复）。"""
        return self._tools_override_var.set(tools)

    def reset_tools_override(self, token) -> None:
        self._tools_override_var.reset(token)

    def _spawn_bg_task(self, coro) -> None:
        """创建后台任务并持有引用（防 GC + 可观察异常）。"""
    ...
    async def process(self, session_id: str, user_input: str, stream: "StreamPort | None" = None) -> dict:
        ...
        try:
            ...
            # 注意：finally 里不能 return——会把 except 之外的异常
            # （包括用户的 abort 取消 CancelledError）静默吞掉。
            try:
                await _s.emit("complete", None)
            except Exception:
                pass
            if _tool_calls_happened:
                self._spawn_bg_task(self._ace_reflect_and_merge(session_id))
            try:
