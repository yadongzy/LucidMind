            ...
            try:
                ...
                for line in extracted.split("\n"):
                    ...
                    if line and len(line) > 5:
                        # md5 而非内建 hash()：内建 hash 受 PYTHONHASHSEED 随机化，
                        # 重启后同一内容会生成不同 key，去重失效产生重复记忆
                        key_digest = hashlib.md5(line.encode("utf-8")).hexdigest()[:4]
                        await self.memory.save(
                            key=f"flush_{today}_{key_digest}",
                            value=line,
                            category="sessions"
                        )
            except Exception as e:
    ...
    def _sanitize_messages(self, messages: list[dict]) -> None:
        ...
        for i, m in enumerate(messages):
            if m.get("role") == "assistant" and m.get("tool_calls"):
                ...
                # 没有 tool_call_id 或找不到对应 assistant.tool_calls 的都是孤儿消息，
                # 必须移除（补假 id 仍会触发 API 400）
