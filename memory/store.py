...
class MemoryStore:
    ...
    def __init__(self, db_path: Path, embedding_dim: int = 768,
        ...
        # timeout/busy_timeout：同库可能被多个连接（learning 适配器 + API）并发访问
    ...
    def _init_schema(self) -> None:
        ...
        if self._vec_enabled:
            try:
                ...
                # distance_metric=cosine：search_vector 的评分公式假设余弦距离 ∈ [0,2]，
                # vec0 默认是 L2（无界），不指定会导致分数/过滤全部失真
                self.db.execute(f"""
                    CREATE VIRTUAL TABLE IF NOT EXISTS memory_vectors
                    USING vec0(id TEXT PRIMARY KEY, embedding float[{self.embedding_dim}] distance_metric=cosine)
                """)
