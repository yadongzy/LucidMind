...
class TokenTracker:
    ...
    def __init__(self):
        # RLock: _save_history 会在 record 持锁期间被重入调用
    ...
    def _save_history(self):
        """保存 token 使用数据（每 50 次调用保存一次）。RLock 允许在 record 持锁期间重入。"""
        with self._lock:
    ...
    def record(self, model: str, provider: str, prompt_tokens: int,
        ...
        # 阈值检查（异步，不阻塞调用）
        self._check_thresholds(day_key, hour_key, total)

    def _check_thresholds(self, day_key: str, hour_key: str, added_tokens: int):
        """检查是否超过 token 预算阈值（日 + 小时维度）。"""
        ...
        if budget > 0:
            ...
            if pct >= self._config.auto_switch_threshold_pct:
                logger.warning(f"🚨 Token 日预算已用 {pct:.1f}%! ({daily_used:,}/{budget:,})")
                ...
                logger.warning(f"⚠️ Token 日预算已用 {pct:.1f}% ({daily_used:,}/{budget:,})")

        # 小时预算（此前配置项存在但从未被检查）
        hour_budget = self._config.max_tokens_per_hour
        if hour_budget > 0:
            hour_used = self._hourly.get(hour_key, 0)
            hour_pct = (hour_used / hour_budget) * 100
            if hour_pct >= self._config.auto_switch_threshold_pct:
                logger.warning(f"🚨 Token 小时预算已用 {hour_pct:.1f}%! ({hour_used:,}/{hour_budget:,})")
                for cb in self._callbacks:
                    try:
                        cb("auto_switch_hourly", hour_pct, hour_used, hour_budget)
                    except Exception:
                        pass
            elif hour_pct >= self._config.warn_threshold_pct:
                logger.warning(f"⚠️ Token 小时预算已用 {hour_pct:.1f}% ({hour_used:,}/{hour_budget:,})")
